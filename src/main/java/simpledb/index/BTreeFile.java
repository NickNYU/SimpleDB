...
public class BTreeFile implements DbFile {
        ...
        try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f))) {
            if (id.pgcateg() == BTreePageId.ROOT_PTR) {
                ...
                if (retval < BTreeRootPtrPage.getPageSize()) {
                    throw new IllegalArgumentException("Unable to read " + BTreeRootPtrPage.getPageSize()
                                                       + " bytes from BTreeFile");
                }
                ...
                if (retval < BufferPool.getPageSize()) {
                    throw new IllegalArgumentException("Unable to read " + BufferPool.getPageSize()
                                                       + " bytes from BTreeFile");
                }
                Debug.log(1, "BTreeFile.readPage: read page %d", id.getPageNumber());
    ...
    public void writePage(Page page) throws IOException {
        ...
        if (id.pgcateg() == BTreePageId.ROOT_PTR) {
            ...
            rf.seek(BTreeRootPtrPage.getPageSize() + (long) (page.getId().getPageNumber() - 1)
                    * BufferPool.getPageSize());
            rf.write(data);
            rf.close();
        }
    ...
    private void transferHalfData(BTreeLeafPage source, BTreeLeafPage dest) throws DbException {
        Iterator<Tuple> tupleIterator = source.reverseIterator();
        Stack<Tuple> stack = new Stack<>();
        int moving = source.getNumTuples() / 2;
        Tuple next = null;
        while (tupleIterator.hasNext() && moving > 0) {
            moving--;
            next = tupleIterator.next();
            source.deleteTuple(next);
            stack.push(next);
        }
        while (!stack.isEmpty()) {
            Tuple tuple = stack.pop();
            dest.insertTuple(tuple);
            source.deleteTuple(tuple);
        }
    }
